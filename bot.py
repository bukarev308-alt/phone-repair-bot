...
def show_summary(message):
    ...
    store_text = "\n".join(
        f"• {s}: {fmt_price(v['revenue'])} грн (чисті: {fmt_price(v['net_revenue'])} грл)" 
...
def generic_handler(message):
    ...
    if txt == "⬅️ Назад":
        ...
        if not new_state:
            ...
            elif new_state == "financial_reports":
                bot.send_message(chat_id, "Повертаємося до фінансових звітів.", 
                               reply_markup=financial_reports_menu())
            else:
    ...
    if state == "stores_management":
        if txt == "➕ Додати магазин":
            ...
            bot.send_message(chat_id, 
                           ...
                           parse_mode="HTML", reply_markup=stores_menu())
            return
        else:
    ...
    # Додавання телефону - вибір магазину
    if state == "add_store":
        if txt == "➕ Додати магазин":
    ...
    # Додавання нового магазину під час додавання телефону
    ...
    if state == "financial_reports":
        if txt == "📊 Тижневий фінансовий звіт":
            ...
            # Спрощений розрахунок тижневого звіту
            ...
            # Спрощений розрахунок місячного звіту
